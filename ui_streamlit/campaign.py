...
from llm_handler import generate_segment_description, generate_marketing_email, generate_marketing_image, return_full_email_prompt, return_full_image_prompt
...
def campaign_generation():
    ...
    with col2:
        ...
        campaign_name = st.text_input("Campaign Name", placeholder="Enter your campaign name", help="Name your campaign for easy identification.")

        # Input for company name
        company_name = st.text_input("Company Name", placeholder="Enter your company name", help="Specify the company for which the campaign is being created.")
        ...
        with col1:
            prompt = st.text_input("Prompt", placeholder="Generate template for prompting", help="Provide a prompt to guide the campaign generation.")
        with col2:
        ...
        with col3:
            ai_generate_prompt = st.button("**AI Generate**", help="Click to generate the campaign using AI.", key="ai_generate_button", use_container_width=True, type="primary", icon="🤖")
            ...
            with st.expander("Settings", expanded=True):  # Display as an expander
                ...
                if selected_option == "Edit Email Prompt":
                    email_prompt = st.text_area(
                        "Edit Email Prompt",
                        value=st.session_state.get("custom_email_prompt", return_full_email_prompt(prompt, company_name, None)),
                        height=150,
                    ...
                    image_prompt = st.text_area(
                        "Edit Image Prompt",
                        value=st.session_state.get("custom_image_prompt", return_full_image_prompt(prompt, company_name)),
                        height=150,
        ...
        if st.session_state.generated_email:
            ...
            st.text_area("Generated Email", value=st.session_state.generated_email, height=300, key="generated_email_area")
